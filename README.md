...
News website — theme **samachar**. No server, no monthly hosting bill.

## Publish (about 15 minutes)

1. **GitHub:** create a new repository named `sachchi-baat` (it can be private). Choose **uploading an existing file** and upload everything in this folder.
...
3. **Domain:** in the Pages project → Custom domains → add `sachchi-baat.pages.dev`.
4. **Admin panel password:** the admin panel asks only for a **पासवर्ड**. That password is a GitHub access token for this one repository:
   ...
   Repository access: **Only select repositories** → `sachchi-baat` → Permissions: **Contents: Read and write** → Expiration: up to 1 year (set a reminder to renew it).
   Copy the token and give it to the client as their password. Open `https://sachchi-baat.pages.dev/admin/` and paste it.
